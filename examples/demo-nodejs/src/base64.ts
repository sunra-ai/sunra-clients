...
const main = async () => {
  try {
    ...
    // convert image url to base64 datauri
    const imageUrl = 'https://assets.sunra.ai/uploads/1751343718083-1751343717640.png'
    const base64Image = await fetch(imageUrl).then((res) => res.arrayBuffer())
    const base64DataUri = `data:image/png;base64,${Buffer.from(base64Image).toString('base64')}`
